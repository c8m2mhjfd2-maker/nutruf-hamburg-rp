...
Players.PlayerAdded:Connect(function(player)
    playerData[player] = {coins=500, level=1, xp=0, capacity=2, recipe=defaultRecipe(), inventory={}, lastScore=0, lastSale=0}
    local leaderstats = Instance.new("Folder"); leaderstats.Name = "leaderstats"; leaderstats.Parent = player
    local coins = Instance.new("IntValue"); coins.Name = "Coins"; coins.Value = 500; coins.Parent = leaderstats
    createPlot(player)
    task.wait(1)
    push(player, "Willkommen in deiner Feuerwerk-Fabrik!")
end)

Players.PlayerRemoving:Connect(function(player) playerData[player] = nil; removePlot(player) end)
...
action.OnServerEvent:Connect(function(player, command, payload)
    ...
    elseif command == "Test" then
        ...
        push(player, "Testshow abgeschlossen: " .. score .. "/100 Punkte.")
